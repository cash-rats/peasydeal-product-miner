...
Implement the crawl flow (check devtools → resolve out dir → run crawler → persist product draft) via RabbitMQ (AMQP) in a **separate worker binary**, running **asynchronously**, with **no application-level retries**.

## Non-goals

- Recreating step observability (per-step UI), orchestration, or durable step checkpoints.
...
1) `check-devtools`
   - Call `chromedevtools.CheckReachable(...)`.
   - On failure: **do not persist** a product draft.

2) `resolve-out-dir`
   - Default `out_dir` to `"out"` when empty.

3) `run-crawler`
   - Call `runner.RunOnce(...)`.
   - On crawler error: **do not fail the step**; return a result shaped as failure so persistence can still happen.

4) `persist-product-draft`
   - Call `ProductDraftStore.UpsertFromCrawlResult(...)` and return `draft_id`.
   - On failure: hard-fail (DLQ).
...
What RabbitMQ does *not* provide and we must compensate for:
...
- `out_dir` is optional; worker defaults to `"out"` when empty (same behavior as the prior flow).
...
**Ack strategy:** ACK only after persistence succeeds (or after we have intentionally decided not to persist, which should match current behavior).
...
2) **check-devtools**
   ...
   - **Do not persist** a product draft on this failure.
...
4) **run-crawler**
   - Call `runner.RunOnce(...)`.
   - If error: log a crawl failure and continue with the failure-shaped result (same as current behavior).
...
- Add a `worker` service that runs `/app/worker` with the `/out` volume if file output is needed.
...
3) Is the existing `ProductDraftStore` intended to be worker-only, or should it live in a shared domain package for other producers?
...
- [x] Create `internal/pkg/amqpclient` with FX-provided connection/channel + `fx.Lifecycle` close hooks
- [x] Create `internal/app/amqp/crawlworker` consumer module (manual ack, `prefetch=1`, DLQ on failures)
