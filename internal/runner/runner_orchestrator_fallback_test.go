...
func TestLoadOrchestratorFinalResult_DefaultSkillNameForTaobao(t *testing.T) {
	t.Parallel()
	...
	final := `{"url":"https://item.taobao.com/item.htm?id=1","status":"ok","captured_at":"2026-01-01T00:00:00Z","title":"t","description":"d","currency":"CNY","price":"1","images":[],"variations":[]}`
	if err := os.WriteFile(filepath.Join(artifactDir, "final.json"), []byte(final), 0o644); err != nil {
		t.Fatalf("write final.json: %v", err)
	}
