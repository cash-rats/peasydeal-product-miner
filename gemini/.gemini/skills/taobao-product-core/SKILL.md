---
name: taobao-product-core
description: Parse Taobao snapshot HTML artifacts offline and return core product fields (title, description, currency, price) as short JSON.
---

# Taobao Product Core (core_extract)
...
2. Parse core fields from HTML/SSR JSON content:
   - `title`
   - `description` (max 1500 chars)
   - `currency`
   - `price`
3. If `s0-snapshot-pointer.json` / `s0-manifest.json` indicates blocked/wall (or wall keywords are detected in html), return `status="needs_manual"`.
